    ...
    <div v-if="showProfile" class="profile-panel">
      ...
      <div class="bet-history">
        ...
        <table v-else class="bet-table">
          ...
          <tbody>
            <tr v-for="bet in pastBets" :key="bet.id" :class="bet.status">
              <td>{{ formatTime(bet.bet_time) }}</td>
              <td>${{ bet.amount.toFixed(2) }}</td>
              <td>
                {{
                  bet.cashout_result ? "$" + bet.cashout_result.toFixed(2) : "—"
                }}
              </td>
              <td>
...
const clickedBeforeTimer = ref(false);
...
async function loadUserData() {
  ...
  if (ud) {
    starterMoney.value = ud.balance;
    totalWagered.value = ud.total_amount_gambled || 0;
  }
  if (misc) userMisc.value = misc;
}
...
async function endGameRound(crashPoint) {
  ...
  await supabase
    .from("THEY CALL IT THE GAME")
    .update({ crash_point: crashPoint, ended_at: new Date().toISOString() })
    .eq("id", currentGameId);
  currentGameId = null;
...
async function placeBetDB(amount) {
  ...
  const { data: ud } = await supabase
    ...
    .maybeSingle();
  if (ud) {
    const newTotal = (ud.total_amount_gambled || 0) + amount;
    await supabase
      .from("user_data")
...
async function resolveBetDB(status, cashoutResult = null) {
  ...
  await supabase
    ...
    .eq("id", currentBetId);
  currentBetId = null;
  if (showProfile.value) await loadPastBets();
}
...
async function onPlayButton() {
  ...
  if (!betActive.value) {
    ...
    if (amount > starterMoney.value) {
      message.value = "Not enough money.";
      return;
    }
